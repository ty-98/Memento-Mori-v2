...
export function ReflectModal({ isOpen, onClose, userData }: ReflectModalProps) {
  ...
  const handleReflect = useCallback(async () => {
    ...
    try {
      // Use the Google Identity Services API directly via popup
      setLoadingMessage('Googleアカウントで認証中...');

      const accessToken = await getGoogleAccessToken();
      if (!accessToken) {
        setError('Googleアカウントの認証がキャンセルされたか、Google Client IDが未設定です。\n.envファイルに VITE_GOOGLE_CLIENT_ID を設定してください。');
        setLoading(false);
        return;
      }
        ...
        >
          ...
          {!summary && !loading && (
            <div className="space-y-6">
              ...
              {error && (
                <div className="text-red-400 text-sm bg-red-400/10 p-4 rounded-lg border border-red-400/20 whitespace-pre-wrap">
                  {error}
                </div>
              )}
...
/**
 * Get a Google access token using Google Identity Services (GIS) popup.
 * This works without @react-oauth/google by using the GIS client library directly.
 */
function getGoogleAccessToken(): Promise<string | null> {
  return new Promise((resolve) => {
    const clientId = (import.meta as any).env?.VITE_GOOGLE_CLIENT_ID;
    if (!clientId || clientId === 'dummy_client_id_for_dev') {
      resolve(null);
      return;
    }
...
function requestToken(clientId: string, resolve: (token: string | null) => void) {
